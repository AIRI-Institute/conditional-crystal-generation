...
def train_epoch(
    ...
    for batch in tqdm(train_dataloader):
        with accelerator.accumulate(model):
            ...
            x_1_coords, element_matrix, elemental_property_matrix, spg, condition, n_sites = x_1_coords.to(
                device), element_matrix.to(device), elemental_property_matrix.to(device), spg.to(
                device), condition.to(device), n_sites.to(device)
